...
def main():
    parser = argparse.ArgumentParser(description="Download GeoJSON files from Earthdata Search")
    parser.add_argument(
        "--output",
        type=str,
    ...
    for granule in results:
        ...
        if any(n in existing_names for n in new_names):
            print(f"{url} の DAAC Scene Names は既存と被っているため、ダウンロードをスキップします。")
            continue
    ...
    print("すべてのGeoJSONのダウンロードが完了しました。ダウンロード件数:", download_count)
    # 最終的な対応関係をCSVとして再出力
    save_records_csv(records, csv_path)
